...
class CMAPI { // this also does not support Last-Modified-Since so i guess just get the routes and update cache very now and then (every minute?)
    ...
    private static let alertsUrlV2 = "\(baseUrlV2)/alerts"
    ...
    func getAlerts() async -> [GtfsRtAlert] {
        var alerts: [GtfsRtAlert] = []
        do {
            alerts = try await NetworkService.makeGETRequest(CMAPI.alertsUrlV2, responseType: [GtfsRtAlert].self)
        } catch {
    ...
    func getLines() async -> [Line] {
        var lines: [Line] = []
        do {
            lines = try await NetworkService.makeGETRequest(CMAPI.linesUrlV2, responseType: [Line].self)
        } catch {
